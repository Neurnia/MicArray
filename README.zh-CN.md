...
├── doc/          # 项目文档与开发笔记
├── hdl/          # 可综合的 HDL 源码（旧 AD7606 逻辑保存在 hdl/deprecated/ 中）
...
- 所有 AD7606 相关 HDL（控制器及测试顶层）已迁入 `hdl/deprecated/`，以便在 `hdl/` 中专注实现新的 INMP441 采集核心，同时保留旧流程供参考。
- UART 通道仍保持畅通：`hdl/test/uart_heartbeat.v` 可继续验证板载 CH340，旧的 ADC 帧发生器已归档到 `hdl/deprecated/test_uart_sim_adc.v`。
...
- `doc/Log.md` 记录开发日志与工作进展（数字麦克风资料参见 `doc/INMP441/INMP441.pdf`）。
...
- “Analog Devices AD7606 8-channel simultaneous sampling ADC module”（作为遗留前端保存在 `hdl/deprecated/` 中）。
- “TXS0108E 8-channel bidirectional level shifter”（与 AD7606 配套的电平转换器，同样归档在 `hdl/deprecated/` 中）。
